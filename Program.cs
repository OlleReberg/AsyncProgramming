...
namespace Exercise_1
{
    public class Unit
    {
        public Task<IPath>? _path;
        private bool _isNotCalculating = true;
        public async void Update()
        {
            _path??= Task.Run(Program.CalculatePath);
            if (_path.IsCompleted)
            {
                _path.Result.Run();
            }
            // if (_path == null && _isNotCalculating)
            // {
            //     _isNotCalculating = false;
            //     _path = await Task.Run(() => Program.CalculatePath());
            //     // if there is no _path - // calculate one using Program.CalculatePath()
            //     _path.Run();
            // }
            // _path?.Run();
