...
namespace Exercise_2
{
    public static class SceneSwitcher
    {
        ...
        public static async Task SwitchScenesAsync(IScene oldScene, IScene newScene)
        {
            ...
            var loadNewSceneTask = newScene.LoadAsync();
            var loadScreen = await Program.LoadLoadingScreenAsync();
            //Wait for loading screen to show
            await loadScreen.ShowAsync();
            var unloadOldScene = oldScene.UnloadAsync();
            //Wait for new scene to load
            await loadNewSceneTask;
            var activateNewScene = newScene.ActivateAsync();
          //Wait for all to finish before hiding loading screen
          Task.WaitAll(unloadOldScene, activateNewScene);
           await loadScreen.HideAsync();
